...
for i in range(len(pms['r'])):
	for j in range(len(pms['rho'])):
		all_results['r%d_rho%d' % (i+1, j+1)] = simulation(method=pms['method'], m=pms['m'], n=pms['n'], 
			r=pms['r'][i], rho=pms['rho'][j], n_p=pms['n_p'], r0=pms['r0'], nrep=pms['nrep'], seed=pms['seed'], 
...
with open(r'result\all_results_sim2_online_v2.pickle', 'wb') as handle:
